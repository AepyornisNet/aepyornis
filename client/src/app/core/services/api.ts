...
import { UserProfile, AppInfo, FullUserProfile, UserUpdateRequest, ProfileUpdateRequest, AppConfig } from '../../core/types/user';
import { Workout, WorkoutDetail, Totals, WorkoutRecord, CalendarEvent } from '../../core/types/workout';
...
import { Statistics, StatisticsParams, GeoJsonFeatureCollection } from '../../core/types/statistics';
...
export class Api {
  ...
  getWorkouts(params?: PaginationParams): Observable<PaginatedAPIResponse<Workout>> {
    ...
    return this.http.get<PaginatedAPIResponse<Workout>>(
      `${this.baseUrl}/workouts`,
      { params: httpParams }
    );
  }
  ...
  updateWorkout(id: number, workout: {
    name?: string;
    date?: string;
    timezone?: string;
    location?: string;
  ...
  refreshWorkout(id: number): Observable<APIResponse<{ message: string }>> {
    return this.http.post<APIResponse<{ message: string }>>(`${this.baseUrl}/workouts/${id}/refresh`, {});
  }

  shareWorkout(id: number): Observable<APIResponse<{ message: string; public_uuid: string; share_url: string }>> {
  ...
  deleteWorkoutShare(id: number): Observable<APIResponse<{ message: string }>> {
    return this.http.delete<APIResponse<{ message: string }>>(`${this.baseUrl}/workouts/${id}/share`);
  }
  ...
  getMeasurements(params?: PaginationParams): Observable<PaginatedAPIResponse<Measurement>> {
    ...
    return this.http.get<PaginatedAPIResponse<Measurement>>(
  ...
  getEquipment(params?: PaginationParams): Observable<PaginatedAPIResponse<Equipment>> {
    ...
    return this.http.get<PaginatedAPIResponse<Equipment>>(
  ...
  getRouteSegments(params?: PaginationParams): Observable<PaginatedAPIResponse<RouteSegment>> {
    ...
    return this.http.get<PaginatedAPIResponse<RouteSegment>>(
      `${this.baseUrl}/route-segments`,
      { params: httpParams }
    );
  }
  ...
  createRouteSegmentFromWorkout(workoutId: number, params: {
    name: string;
    start: number;
    end: number;
  ...
  updateRouteSegment(id: number, params: {
    name: string;
    notes: string;
  ...
  deleteRouteSegment(id: number): Observable<APIResponse<{ message: string }>> {
    return this.http.delete<APIResponse<{ message: string }>>(`${this.baseUrl}/route-segments/${id}`);
  }

  refreshRouteSegment(id: number): Observable<APIResponse<{ message: string }>> {
    return this.http.post<APIResponse<{ message: string }>>(`${this.baseUrl}/route-segments/${id}/refresh`, {});
  }

  findRouteSegmentMatches(id: number): Observable<APIResponse<{ message: string }>> {
    return this.http.post<APIResponse<{ message: string }>>(`${this.baseUrl}/route-segments/${id}/matches`, {});
  }
  ...
  // Calendar endpoints
  getCalendarEvents(params?: { start?: string; end?: string; timeZone?: string }): Observable<APIResponse<CalendarEvent[]>> {
    let httpParams = new HttpParams();
    if (params?.start) {
    ...
    return this.http.get<APIResponse<CalendarEvent[]>>(
      `${this.baseUrl}/workouts/calendar`,
      { params: httpParams }
    );
  }
}
